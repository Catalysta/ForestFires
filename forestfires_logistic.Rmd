...
#transform FFMC
train2$FFMC <- log(-train2$FFMC+ max(train2$FFMC)+1)
colnames(train2)[6]<-"TrFFMC"
...
cd<-cooks.distance(glm(large~.,dat=train2,family="binomial"))
which(cd>qf(0.5,ncol(train2)+1,nrow(train2)-ncol(train2)-1)) #these also look fine
```
